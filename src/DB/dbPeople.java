...
public class dbPeople {
    ...
    //添加people信息操作
    public boolean saveInfo(peoplePO pPO){
  ...
  //people条件查找操作
    public List findInfo(String type,Object value){
    	getSession=new HibernateSessionFactory();
  	   ...
  	   try {
		transaction=session.beginTransaction();
		String queryString="from peoplePO as model where model."+type+"=?";
    ...
    public List findAllInfo(){
    	getSession=new HibernateSessionFactory();
  	   ...
  	   try {
		transaction=session.beginTransaction();
		String queryString="from peoplePO";
		query=session.createQuery(queryString);		
		List list=query.list();
		transaction.commit();
		session.close();
    ...
    public boolean deleteInfo(String id) {
    	 getSession=new HibernateSessionFactory();
    	   ...
    	   try {
  		      transaction=session.beginTransaction();
  		    peoplePO infoPeoplePO=new peoplePO();
  		    infoPeoplePO=(peoplePO)session.get(peoplePO.class,id);
    ...
    //修改people信息
    public boolean updateInfo(peoplePO pPo){
 	   getSession=new HibernateSessionFactory();
 	   session=getSession.getSession();
 	   try {
 		     transaction=session.beginTransaction();
